    ...
    <v-card>
      <v-card-title style="height: 45px; background-color: #3f51b5">
        <v-layout align-center class="fill-height">
          ...
          <v-spacer/>
          <v-btn icon small @click="editDialog = false"><v-icon color="white">mdi-close</v-icon></v-btn>
        </v-layout>
      </v-card-title>
      <v-divider/>
      <v-card-text class="px-2 py-4" style="max-height: 350px; overflow-y: auto">
...
export default {
  ...
  data () {
    return {
      rules: {
        required: value => (!!value && value.trim().length >= 2) || 'Total character >= 2',
      },
      valid: false,
      editDialog: false,
      form: {}
