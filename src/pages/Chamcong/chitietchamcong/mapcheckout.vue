<template>
  <div class="dashboard-map-container-checkout" id="dashboard-map-container-checkout">
    <div class="dashboard-map-checkout" id="dashboard-map-checkout"></div>
  </div>
</template>
...
export default {
  ...
  methods: {
    ...
    getMap(lat = 21.005796351058695, lng = 105.8465337753296) {
      this.map = L.map("dashboard-map-checkout").setView([lat, lng], 18);
...
<style scoped>
.dashboard-map-container-checkout {
  min-width: 0;
  min-height: 0;
  flex: 1;
}

.dashboard-map-checkout {
  width: 100%;
  height: 280px;
}
#boxsearch {
